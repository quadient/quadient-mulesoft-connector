...
| Api token a| String |  Bearer token for authenticating your request. You need to generate an API key for a user role with relevant permissions in Settings | Aministration | API Keys. |  | *x*{nbsp}
...
| Holder Value a| String |  Define the holder of the ticket. |  | *x*{nbsp}
...
|======================
| Field | Type | Description | Default Value | Required
| Categorizations a| Array of <<CategorizationConditionFe>> |  |  | 
| Conditions a| Array of String |  |  | 
| Metadata a| Array of <<MetadataConditionFe>> |  |  | 
